...
public class SI {
	...
	public int[][] connectedMatrix;//亲密度关系矩阵
	...
	public int[][] readInf_4(Graph1 network,ArrayList<String> obv_set){
		...
		for (String working_name : obv_set) {
			...
			String filePath="G:\\Data Files\\dblp_large\\v"+obv.origin+".txt";//相当于按照obv对应的index中：每行代表一个working sensor的direction info
			try {
	...
	public int[][] readInf_4_from_oneFile(Graph1 network,ArrayList<String> obv_set){
		int[][] short_temp=new int[obv_set.size()][network.verNum];
		String filePath="/Users/houdongpeng/1/ComplexNetwork/src/quick_J_0117/github_shortestPath.txt";
		try {
