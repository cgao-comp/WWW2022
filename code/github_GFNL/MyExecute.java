...
public class MyExecute {
	...
	public static void initNet_heterogeneous_combinedFONC_divideWeight(int type) throws Exception {
		...
		String path="G:\\eclipse workspace\\1\\ComplexNetwork\\src\\results.txt";
...
//ELSE:
						mySi.initNet_SIR_Greedy_fromFile(network, infected_p, 0, center_Num, 
								"G:\\Data Files\\OBV_5-40\\facebook"+allObvNum_rate+".txt");
...
//						System.out.println("距离矩阵生成完成......");
						if(ALG.isBigScaleNet) {
							//是大型网络需要从文件中读取------------此代码段适用于一般大的文件----从一个文件中即可读出信息
							...
							//是大型网络需要从文件中读取------------此代码段适用于特别大的文件----从几万个文件中选取
//							mySi.readInf_4(network, obv_set);
						}else {
							//非大型网络直接在内存中已经有信息了
						}
